...
const PlaceOrder = () => {
    ...
    const placeOrder = async (e) => {
        ...
        // 使用 forEach 替代 map，因为你只是遍历，并不需要生成新数组
        food_list.forEach(item => {
            if (cartItems[item._id] > 0) {
                let itemInfo = { ...item };  // 拷贝对象，避免修改原始对象
        ...
        let orderData = {
            address: data,
            items: orderItems,
            amount: getTotalCartAmount() + 5,  // 这里 +5 可能是配送费等
        };
        ...
        try {
            ...
            if (response.data.success) {
                ...
                window.location.replace(session_url);  // 跳转到支付页面或订单确认页面
            } else {
